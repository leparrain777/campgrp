...
allmodes = flipud(sum(nmodes(:,1),2));
...
%
tmax = ndxTime(maxs(2:end-1,1));
maxEO = maxs(2:end-1,2);
tmin = ndxTime(mins(2:end-2,1));
minEO = mins(2:end-2,2);
%
...
suptitle(sprintf('SM90 Run %g Oblituity Pacing for u = %g, p = %g',runID,param(5),param(1))); 
