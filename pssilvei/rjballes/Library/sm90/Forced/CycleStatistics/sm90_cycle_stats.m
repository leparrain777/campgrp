...
for i=1:length(tmin_co2)-1
   ...
   %[idxMaxI,idxMinI,partialsper] = sm90_partialmelts(min(find(t>=tmax_co2(i))),tspan(i+1),I);
   %num_pmelts = num_pmelts+numPartials;
   %melts_per_cycle = [melts_per_cycle numPartials];
   %idMax_parI = [idMax_parI idxMaxI];
   %idMin_parI = [idMin_parI idxMinI];
   
   %%Constructs a vector of the times until a partial melt from start of cycle
   %tcycle_pmelts = [];
   %for j=1:length(idxMaxI)
   %   pmelt_trig = t(idxMaxI(j)) - tmin_co2((i));
   %   tcycle_pmelts = [tcycle_pmelts pmelt_trig];
   %end %for
   %pmelt_times = [pmelt_times 10*tcycle_pmelts];
...
[r16,p16] = corrcoef(ye_min(:,1),ye_max(:,1));  %Ice at melt to Ice at cool

[r17,p17] = corrcoef(ye_max(1:end-1,1),ye_min(2:end,1));  %Ice at cooling to Ice at next melt
