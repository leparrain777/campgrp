...
runID = 23;
cycleNum = 24;  %Number of cycle want to plot
...
p = size(Mu_unfor,1);
for i=2:p
   if -p.*Theta_unfor(i) + 0.9*Mu_unfor(i) + Theta_unfor(i).^2 - 0.5.*Mu_unfor(i).*Theta_unfor(i) - Mu_unfor(i).*Theta_unfor(i).^2 > 0
      redt_mu_unfor = [redt_mu_unfor i];
   elseif -p.*Theta_unfor(i) + 0.9*Mu_unfor(i) + Theta_unfor(i).^2 - 0.5.*Mu_unfor(i).*Theta_unfor(i) - Mu_unfor(i).*Theta_unfor(i).^2 < 0
...
if ydotFlag
   ...
   for h = X
      for i = Y
         for j = Z
            if -p*j + 0.9*i + j.^2 - 0.5.*i.*j - i.*j.^2 > 0
               red_ydot = [red_ydot; h i j];
            elseif -p*j + 0.9*i + j.^2 - 0.5.*i.*j - i.*j.^2 < 0
   ...
   sm90_plotcycle3D(runID,descr,cycleNum,cycleNum)
   
   syms x y z
   h1 = fimplicit3(-1.1*z+0.9.*y+z.^2-0.5.*y.*z-y.*z.^2);
   h1.FaceAlpha = 0.25;
   ylabel('Theta')
   xlabel('Mu')
   zlabel('I')
   ...
   suptitle('Phase Space Coloring Based on YDot (red-pos.; blue-neg.)') 
...
if zdotFlag
   ...
   syms x y z
   h1 = fimplicit3(-p*z+0.9.*y+z.^2-0.5.*y.*z-y.*z.^2);
   h1.FaceAlpha = 0.5;
   ylabel('Theta')
   xlabel('Mu')
   zlabel('I')
   ...
   suptitle('Phase Space Coloring Based on ZDot (red-pos.; blue-neg.)')
