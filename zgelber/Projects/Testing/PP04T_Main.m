function [tout,Vout,Aout,Cout,Hout,Fout,NorthF] = PP04T_Main(Constants,insol,n0,tstart,tfinal)
...
tA=Constants(3);
%Misc
force_amplitude = Constants(16);
%Constants
x=Constants(4);
y=Constants(5)*force_amplitude;
z=Constants(6);
al=Constants(7)*force_amplitude;
be=Constants(8);
ga=Constants(9);
de=Constants(10);
a=Constants(11);
...
if insol == 6
    time1 = [tstart:tfinal];
    NorthF = (k1*sin((2*pi/41)*time1))+(k2*sin((2*pi/23)*time1));
    NorthF = NorthF';
end
timecheck = [0:9]';
