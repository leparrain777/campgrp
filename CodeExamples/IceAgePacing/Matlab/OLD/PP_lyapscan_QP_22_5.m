...
p.kt1=1.0;
%scan parameter p.kt2 min and max
ktm=0.0;
ktp=1.0;
...
imagesc(taus,kts,ly,[-0.015,0.015]);
%imagesc(tulcs,gammas,ly,[-0.05,0.05]);
axis xy;
xlabel('\tau');
%xlabel('T_{ULC}');
ylabel('kt2');
ttext=sprintf('PP LEs QP tsteps=%d ttrans=%d (xs,ys)=(%d,%d)',tsteps,ttrans,xsteps,ysteps);
title(ttext);
colorbar

ftt=sprintf('PP_QP_ts_%d__tt_%d_xs_%d__ys_%d',tsteps,ttrans,xsteps,ysteps);
