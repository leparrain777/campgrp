...
%xlabel('T_{ULC}');
ylabel('\gamma');
ttext=sprintf('scan of lyap exponents, tsteps=%d ttrans=%d (xs,ys)=(%d,%d) alpha=%d beta=%d',tsteps,ttrans,xsteps,ysteps,p.alpha,p.beta);
title(ttext);
colorbar

ftt=sprintf('dSCW_lyapscan_ts_%d__tt_%d_xs_%d__ys_%d_al_%d_be_%d',tsteps,ttrans,xsteps,ysteps,p.alpha,p.beta);
