...
%xlabel('T_{ULC}');
ylabel('\gamma');
ttext=sprintf('LEs QP tsteps=%d ttrans=%d (xs,ys)=(%d,%d) alpha=%g beta=%g delta=%g',tsteps,ttrans,xsteps,ysteps,p.alpha,p.beta,p.delta);
title(ttext);
colorbar

ftt=sprintf('dSCWLEs_QP_ts_%d__tt_%d_xs_%d__ys_%d_al_%g_be_%g_de_%g.eps',tsteps,ttrans,xsteps,ysteps,p.alpha,p.beta,p.delta);
...
F=p.gamma*sin(t*p.omega)+p.gamma2*sin(t*p.omega2);
temp=1/p.tau;
% nonlinear ODE
