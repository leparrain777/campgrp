    ...
    switch nIC
        case 1
            ...
            plot(tt(:),IceVol(:),'r',tt,AtmCO2(:),'b',tt(:),OcTemp(:),'k');
            ylabel('y');
        ...
        otherwise
            ...
            for irun = 1:mruns
                ...
                subplot(4,1,1);
                % plot model state variables
                plot(tt(:),IceVol(:,irun),'r',tt(:),AtmCO2(:,irun),'b',tt(:),OcTemp(:,irun),'k');
                ylabel('y');
                title(sprintf('kt1=%.4f kt2=%.4f',p.kt1,p.kt2));
                %
                %lyap 
            ...
            clf;
            %
            H=plot3(repmat(tt,[1 nIC]),IceVol,AtmCO2);
            xlabel('t');
            ylabel('V');
            zlabel('A');
            xlim([tt(1) tt(end)])
            ...
            mrun = 2
            for j=1:2
                H=plot3(IceVol(:,j),AtmCO2(:,j),OcTemp(:,j),...
                    'Color', ColorOrder(ColorIndex, :));
                ColorIndex = ColorIndex+1;
                if (ColorIndex > mColor)
                    ColorIndex = 1;
